...
userpass(){
  nano /etc/openvpn/auth/users.pass
}
confautodel(){
			number_of_clients=$(tail -n +2 /etc/openvpn/server/easy-rsa/pki/index.txt | grep -c "^V")
			if [[ "$number_of_clients" = 0 ]]; then
...
mkdir -p /etc/openvpn/auth/tmp
git clone https://github.com/Felytweb/auth && cd auth
mv users.pass /etc/openvpn/auth/
mv verify.sh /etc/openvpn/auth/
...
verify-client-cert none
username-as-common-name
tmp-dir /etc/openvpn/auth/tmp" >> /etc/openvpn/server/config.conf
